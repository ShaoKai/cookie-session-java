...
public class VelocityMultipleLayoutViewResolver extends VelocityViewResolver {
	...
	/**
	 * Set the context key used to specify an alternate layout to be used instead of the default layout. Screen content templates can override the layout template that they wish to be wrapped with by
	 * setting this value in the template, for example:<br>
	 * <code>#set( $layout = "MyLayout.vm" )</code>
	 * <p>
	 * The default key is "layout", as illustrated above.
	 * 
	 * @param layoutKey
	 *            the name of the key you wish to use in your screen content templates to override the layout template
	 ...
	 * <p>
	 * Default is "screen_content": accessed in VTL as <code>$screen_content</code>.
	 * 
	...
	protected AbstractUrlBasedView buildView(final String viewName) throws Exception {
		...
		if (!this.mappings.isEmpty()) {
			for (Map.Entry<String, String> entry : this.mappings.entrySet()) {
				...
				String velocityViewName = viewName.startsWith("/") ? viewName.substring(1) : viewName;

				// If the view matches the regexp mapping
				if (velocityViewName.matches(mappingRegexp)) {
